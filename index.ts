import init, { CollisionAlgorithm, Particle, Vec2, World } from "./pkg/collisions.js";
...
let mousedown = false;
let phantomParticle: PhantomParticle | null = null;
let segments: number[][] = [];
...
const renderLoop = () => {
  const alg: CollisionAlgorithm = parseInt(document.querySelector<HTMLInputElement>('input[name="collision"]:checked')?.value || "0");
...
function render(memory: WebAssembly.Memory, world: World) {
  ...
  if (phantomParticle) {
    fillCircle(
      phantomParticle.posX,
      phantomParticle.posY,
...
function addSegment(start1: number, end1: number, start2: number, end2: number) {
  segments.push([start1, end1, start2, end2]);
  world.push_segment(Vec2.new(start1, end1), Vec2.new(start2, end2));
}

function tryAddParticle(p: Particle) {
  if (world.try_push(p)) {
    totalParticles++;
  }
}
